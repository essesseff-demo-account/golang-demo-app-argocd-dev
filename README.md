# golang-demo-app - Argo CD Application (DEV)

This repository contains the Argo CD Application manifest for the **DEV** environment of the golang-demo-app essesseff™ app.  
...
```
golang-demo-app-argocd-dev/
├── app-of-apps.yaml.template                      # Root Application template
├── argocd/
│   └── golang-demo-app-dev-application.yaml.template  # DEV environment Application manifest (auto-synced) template
...
│   └── offboard-golang-demo-app-dev.sh   # script for offboarding the golang-demo-app dev app 1) from essesseff only or 2) from Argo CD and K8s entirely
├── setup-argocd-cluster.sh           # Argo CD K8s setup script 
├── setup-argocd.sh                   # Argo CD golang-demo-app-dev essesseff app setup script 
└── README.md                         # This file
```
...
### Configure golang-demo-app-dev essesseff App to Argo CD and deploy to K8s

***Note: it is highly recommended that, rather than going through all the steps here to configure Argo CD, that for convenience and faster turnaround time you instead use the [essesseff-onboarding-utility](https://github.com/essesseff/essesseff-onboarding-utility)***

1. **Git Clone This Repository**:
   ```bash
   git clone git@github.com:essesseff-demo-account/golang-demo-app-argocd-dev.git
   ```
...
2. **Configure Environment Variables in .env File**:
   ...
   Then set the environment variables in the .env which will be used for generating the following from the templates in this repository:
      ...
      - `golang-demo-app-argocd-dev` repository (to read Application manifests)
      - `golang-demo-app-config-dev` repository (to read Helm charts and values)
      ...
      c. ***Configuration of golang-demo-app-dev Deployment in Argo CD***:

      golang-demo-app-dev-application.yaml is used to configure the golang-demo-app-dev deployment

      d. ***Configuration of golang-demo-app-argocd-dev App-of-Apps Deployment in Argo CD***:

      app-of-apps.yaml is used to configure the golang-demo-app-argocd-dev app-of-apps deployment

      e. ***Configuration of golang-demo-app-dev Argo CD Notifications to essesseff***:

      If notifications-secret.yaml is downloaded from essesseff for golang-demo-app-dev, notifications-configmap.yaml will be used to configure Argo CD notifications to essesseff.
...
4. **Run the setup-argocd.sh Script**:
   ...
   This script applies all secrets, configmaps, Argo CD application definitions, etc. for golang-demo-app DEV.
...
5. **Verify in Argo CD UI**:
   ...
   You should see:
   - `golang-demo-app-argocd-dev` - Root Application (watches this repository)
   - `golang-demo-app-dev` - Environment Application (auto-synced by root Application)

6. **Access the Deployed Application**:
   ```bash
   kubectl port-forward service/golang-demo-app-dev 8081:80 -n essesseff-demo-account
   # Access: http://localhost:8081
   ```
### How to Offboard golang-demo-app-dev Deployment from Argo CD and K8s

1. **Execute the offboarding script**:
   ```bash
   cd offboarding
   chmod 744 offboard-golang-demo-app-dev.sh
   ./offboard-golang-demo-app-dev.sh
   ```
...
- **Name**: `golang-demo-app-dev`
- **Namespace**: `argocd`
- **Source Repository**: `golang-demo-app-config-dev`
...
1. Push code to `main` branch in `golang-demo-app` source code repository
2. GitHub Actions builds container image
3. essesseff GitHub App automation triggers essesseff to auto-update Helm `golang-demo-app-config-dev/Chart.yaml` and `golang-demo-app-config-dev/values.yaml` with the image tag of the newly built image
4. Argo CD syncs DEV Application automatically on K8s

### Manual GitOps Deployment

1. Push update(s) to the `main` branch in `golang-demo-app-config-dev` repository, typically to Helm Chart.yaml and/or values.yaml
2. Argo CD syncs DEV Application automatically on K8s

## Repository URLs

- **Source**: `https://github.com/essesseff-demo-account/golang-demo-app`
- **Config DEV**: `https://github.com/essesseff-demo-account/golang-demo-app-config-dev`
- **Argo CD DEV**: `https://github.com/essesseff-demo-account/golang-demo-app-argocd-dev` (this repo)
...
- Allow Argo CD to detect changes in `argocd/golang-demo-app-dev-application.yaml` more quickly
